...
public class ScheduleController {
    ...
    public String calendar(@PathVariable(required = false) String arrow, HttpSession session, Model model){
        ...
        String date = "일 월 화 수 목 금 토".split(" ")[today.getDayOfWeek().getValue() % 7];      // 요일 추가하는 코드 7로 나눠줘야 0,1,2,3...7로 나옴
        String sessionMonthYear = (String) session.getAttribute("scheduleMonthYear");       //2024.03.xx
        if(sessionMonthYear != null){
            year = Integer.parseInt(sessionMonthYear.substring(0, 4));
            month = Integer.parseInt(sessionMonthYear.substring(5));   // 끝은 표시 안해도됨.
        }
        if (arrow != null){
            switch (arrow){
            case "left":
        ...
        if (startDate != 0){
            ...
            for (int i = 0; i < startDate; i ++){       // 지난달 날짜 표시
                sdate = String.format("%d%02d%02d", prevYear, prevMonth, prevDay+i);
                SchDay sd = new SchDay();
                sd.setDay(prevDay+i);
                sd.setDate(i);
                sd.setSdate(sdate);
                week.add(sd);           // 지날달에서 지난달 말일까지 채워줌
            }
        }
        for (int i = startDate, k = 1; i < 7; i++, k++){            // 이번달 데이터
